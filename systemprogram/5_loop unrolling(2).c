...
//put words on a file to linked list
 node* CreateNode(char *fileName, node*head){
	...
	if ((text = fopen("/home/leesoonju/바탕화면/4 - loop unrolling(2)/file1.txt", "r")) == NULL) {
        printf("Can not find file: %s", fileName);
        exit(0);
    	}
    }
    else if (fileName == "file2.txt"){
        if ((text = fopen("/home/leesoonju/바탕화면/4 - loop unrolling(2)/file2.txt", "r")) == NULL) {
        printf("Can not find file: %s", fileName);
        exit(0);
        }
    }
...
if(isEmpty == 1){
 ...
 node* findCommmonWords(node*head1, node*head2, node*inputHead){
    ...
    for(iterator1=head1;iterator1->next->word!=NULL;iterator1 = iterator1->next){
        ...
        for(iterator2a = head2, iterator2b = head2->next; iterator2b->next->next->next->word!=NULL; iterator2a = iterator2b->next, iterator2b = iterator2a->next){
