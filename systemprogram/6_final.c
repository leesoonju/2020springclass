...
//put words on a file to linked list
 node* CreateNode(char *fileName, node*head){
	...
	if ((text = fopen("/home/leesoonju/바탕화면/5 - memory access/file1.txt", "r")) == NULL) {
        printf("Can not find file: %s", fileName);
        exit(0);
    	}
    }
    else if (fileName == "file2.txt"){
        if ((text = fopen("/home/leesoonju/바탕화면/5 - memory access/file2.txt", "r")) == NULL) {
        printf("Can not find file: %s", fileName);
        exit(0);
        }
    }
...
node* find2Grams(node*head1, node*head2, node*inputHead){
    ...
    node *iterator2b;
    node *iterator3 = inputHead;
    char* case1, *case2a, *case2b;
    char* case1n, *case2an, *case2bn; //next
    int i;
    ...
    for(iterator1=head1, case1 = iterator1->word, case1n = iterator1->next->word; iterator1->next->next->word!=NULL;
        iterator1 = iterator1->next, case1 = iterator1->word, case1n = iterator1->next->word){
        //loop through second list elements
        for(iterator2a = head2, iterator2b = head2->next, case2a = iterator2a->word, case2b = iterator2b->word,
            case2an = iterator2a->next->word, case2bn = iterator2b->next->word; iterator2b->next->next->next->word!=NULL;
            iterator2a = iterator2b->next, iterator2b = iterator2a->next, case2a = iterator2a->word, case2b = iterator2b->word,
            case2an = iterator2a->next->word, case2bn = iterator2b->next->word){

            if (Strcmp(case1, case2a) == 0  && Strcmp(case1n, case2an) ==0){
                Strcpy(iterator3->word ,case1);
                Strcat(iterator3->word," ");
                Strcat(iterator3->word, case1n);
                iterator3->next = (struct Node*)Malloc(sizeof(struct Node)*2+1);
                iterator3 = iterator3->next;
                isEmpty = 0;
                break;
            }
            if (Strcmp(case1, case2b) == 0 && Strcmp(case1n, case2bn) ==0){
                Strcpy(iterator3->word ,case1);
                Strcat(iterator3->word," ");
                Strcat(iterator3->word, case1n);
                iterator3->next = (struct Node*)Malloc(sizeof(struct Node)*2+1);
...
void printList(node *head){
  FILE * fp = fopen("/home/leesoonju/바탕화면/5 - memory access/result.txt", "w");	
  do{
        fputs(head->word, fp);
	fputs("  ", fp);
        head = head->next;
...
void printList2(node *head){
  FILE * fp = fopen("/home/leesoonju/바탕화면/5 - memory access/result2.txt", "w");
  do{
        fputs(head->word, fp);
        fputs("\n ", fp);
        head = head->next;
