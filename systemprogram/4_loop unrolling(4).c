...
//put words on a file to linked list
 node* CreateNode(char *fileName, node*head){
	...
	if ((text = fopen("/home/leesoonju/바탕화면/3 - loop unrolling(4)/file1.txt", "r")) == NULL) {
        printf("Can not find file: %s", fileName);
        exit(0);
    	}
    }
    else if (fileName == "file2.txt"){
        if ((text = fopen("/home/leesoonju/바탕화면/3 - loop unrolling(4)/file2.txt", "r")) == NULL) {
        printf("Can not find file: %s", fileName);
        exit(0);
        }
    }
...
node* find2Grams(node*head1, node*head2, node*inputHead){
    ...
    for(iterator1=head1; iterator1->next->next->word!=NULL; iterator1 = iterator1->next){
        //loop through second list elements
        for(iterator2a = head2, iterator2b = head2->next, iterator2c = head2->next->next, iterator2d = head2->next->next->next;
	    iterator2d->next->next->next->next->next->word!=NULL;
      	    iterator2a = iterator2d->next, iterator2b = iterator2a->next, iterator2c = iterator2b->next, iterator2d = iterator2c->next){
            ...
            if (Strcmp(iterator1->word, iterator2c->word) == 0 && Strcmp(iterator1->next->word, iterator2c->next->word) ==0){
                Strcpy(iterator3->word ,iterator1->word);
                Strcat(iterator3->word," ");
                Strcat(iterator3->word, iterator1->next->word);
                iterator3->next = (struct Node*)Malloc(sizeof(struct Node)*2+1);
	    ...
	    if (Strcmp(iterator1->word, iterator2d->word) == 0 && Strcmp(iterator1->next->word, iterator2d->next->word) ==0){
                Strcpy(iterator3->word ,iterator1->word);
                Strcat(iterator3->word," ");
                Strcat(iterator3->word, iterator1->next->word);
                iterator3->next = (struct Node*)Malloc(sizeof(struct Node)*2+1);
...
void printList(node *head){
  FILE * fp = fopen("/home/leesoonju/바탕화면/3 - loop unrolling(3)/result.txt", "w");	
  do{
        fputs(head->word, fp);
	fputs("  ", fp);
        head = head->next;
...
void printList2(node *head){
  FILE * fp = fopen("/home/leesoonju/바탕화면/3 - loop unrolling(3)/result2.txt", "w");
  do{
        fputs(head->word, fp);
        fputs("\n ", fp);
        head = head->next;
