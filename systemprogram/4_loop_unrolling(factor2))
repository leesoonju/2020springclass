...
//put words on a file to linked list
 node* CreateNode(char *fileName, node*head){
	...
	if ((text = fopen("/home/leesoonju/바탕화면/3 - loop unrolling(2)/file1.txt", "r")) == NULL) {
        printf("Can not find file: %s", fileName);
        exit(0);
    	}
    }
    else if (fileName == "file2.txt"){
        if ((text = fopen("/home/leesoonju/바탕화면/3 - loop unrolling(2)/file2.txt", "r")) == NULL) {
        printf("Can not find file: %s", fileName);
        exit(0);
        }
    }
...
void printList(node *head){
  FILE * fp = fopen("/home/leesoonju/바탕화면/3 - loop unrolling(2)/result.txt", "w");	
  do{
        fputs(head->word, fp);
	fputs("  ", fp);
        head = head->next;
...
void printList2(node *head){
  FILE * fp = fopen("/home/leesoonju/바탕화면/3 - loop unrolling(2)/result2.txt", "w");
  do{
        fputs(head->word, fp);
        fputs("\n ", fp);
        head = head->next;
